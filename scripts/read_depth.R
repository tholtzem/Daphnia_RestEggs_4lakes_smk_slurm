...
getwd()
setwd("/scratch/c7701178/mach2/DAPHNIA/Daphnia_RestEggs_4lakes_smk_slurm/")
...
for (i in 1:nrow(df)){
    ...
    mean_depth_DP10 <- mean(depth[depth > 10])
    mean_depth_DP20 <- mean(depth[depth > 20])
    mean_depth_DP30 <- mean(depth[depth > 30])
  ...
  # Bind stats into dataframe and store sample-specific per base depth and presence data
    if (i==1){
      output <- data.frame(bamfile, mean_depth, sd_depth, mean_depth_nonzero, mean_depth_within2sd, median, proportion_of_reference_covered, mean_depth_DP10, mean_depth_DP20, mean_depth_DP30)
      total_depth <- depth
      total_presence <- presence
    } else {
      output <- rbind(output, cbind(bamfile, mean_depth, sd_depth, mean_depth_nonzero, mean_depth_within2sd, median, proportion_of_reference_covered, mean_depth_DP10, mean_depth_DP20, mean_depth_DP30))
