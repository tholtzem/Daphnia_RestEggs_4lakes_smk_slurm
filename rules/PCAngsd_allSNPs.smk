...
rule PCAngsd:
  ...
  shell:
    ...
    for i in {{2..10}}; do
    	singularity exec --home $PWD:$HOME /scratch/c7701178/bio/ngs+tools.sif pcangsd --beagle angsd/{wildcards.sets}/angsd_GL{wildcards.GL}_minInd{wildcards.IND}_maf{wildcards.minMaf}_minDepth{wildcards.MinDepth}_maxDepth{wildcards.MaxDepth}.beagle.gz --out pcangsd/{wildcards.sets}/PCAngsd_GL{wildcards.GL}_minInd{wildcards.IND}_maf{wildcards.minMaf}_minDepth{wildcards.MinDepth}_maxDepth{wildcards.MaxDepth} --maf {wildcards.minMaf} --admix --admix_K $i --n_eig 10 --threads {threads} 2> {log}
    done
    """
